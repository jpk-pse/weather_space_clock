// ═══════════════════════════════════════════════════════
//  api.ino — Anthropic rate-limit header fetch
// ═══════════════════════════════════════════════════════
...
void fetchUsage(const char* tok, UsageData& out) {
    out.ok = false;
    ...
    https.addHeader("Authorization",    String("Bearer ") + tok);
    ...
    https.collectHeaders(RL_HEADERS, 4);
    ...
    Serial.printf("[API] 5h=%.2f 7d=%.2f\n", h5u.toFloat(), d7u.toFloat());

    if (h5u.length() == 0 && d7u.length() == 0) {
        if (code == 401) strlcpy(out.error, "auth_failed", sizeof(out.error));
        else snprintf(out.error, sizeof(out.error), "no_headers_%d", code);
        return;
    }
